...
// This will auto-reconnect if the wallet has existing permissions
  ...
  // Set up wallet event listeners (ONCE for entire app)
  useEffect(() => {
    ...
    const handleChainChanged = async (chainId: string) => {
      // Wallets recommend reloading the page on chain change
    ...
    try {
      ...
      if (err.message?.includes("rejected")) {
        ...
        error("No wallet detected", {
          description: "Please install an EVM-compatible wallet to connect.",
          action: {
  ...
  /**
   * Request user to switch to a different wallet account
  ...
  const switchWalletAccount = useCallback(async () => {
    try {
      ...
      // Request account switch via wallet picker
